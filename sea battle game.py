...
class Board: # Класс игровое поле
    ...
    def __str__(self): # Игровое поле обозначение
        res = ""
        res += "  | 1 | 2 | 3 | 4 | 5 | 6 |"
    ...
    def out(self, d):
        return not ((0 <= d.x < self.size) and (0 <= d.y < self.size)) # Точка за пределами поля

    def shot(self, d): # Стельба по доске
        if self.out(d):
...
class AI(Player): # Класс игрок - компьютер(Искуственный интелект)
    def ask(self):
